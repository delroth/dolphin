...
IPCCommandResult WFSSRV::IOCtl(const IOCtlRequest& request)
{
  ...
  case IOCTL_WFS_CHDIR:
    m_current_directory = Memory::GetString(request.buffer_in + 2, Memory::Read_U16(request.buffer_in));
  ...
  case IOCTL_WFS_GET_HOMEDIR:
    ...
    Memory::Write_U16(m_home_directory.size(), request.buffer_in);
    Memory::CopyToEmu(request.buffer_in + 2, m_home_directory.data(), m_home_directory.size());
    break;
